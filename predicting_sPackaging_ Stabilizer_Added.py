# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 16:20:37 2019
...
test_data = df[df['Packaging_ Stabilizer_Added'].isnull()]
train_data = df[df['Packaging_ Stabilizer_Added'].notnull()]



train_data = train_data.drop(['Sample_ID','Transparent_ Window_in_Package','Preservative_Added'], axis =1)
test_data = test_data.drop(['Sample_ID','Transparent_ Window_in_Package','Preservative_Added','Packaging_ Stabilizer_Added'], axis =1)

x= train_data.drop(columns ='Packaging_ Stabilizer_Added')
#x = x.drop(columns = 'Difference_From_Fresh' )
y = train_data.loc[:,['Packaging_ Stabilizer_Added']]
...
test_data['Packaging_ Stabilizer_Added']=y_predicted
...
export_csv = new_data.to_csv(r'C:\Users\sb00747428\Downloads\pepsico challenge\pckg_stb_add.csv', index = None, header=True)